...
var X = 800, Y=500; var game = new Phaser.Game(X, Y, Phaser.CANVAS, 'game', {preload: preload, create: create, update: update, 
render: render} ); var floor1, floor2, platform1, counter = 0; var text; var ispravanUnos = false; var player, upKey, 
downKey, leftKey, rightKey; function preload() {
...
function update() { game.physics.arcade.collide(player, [floor1, floor2, platform1]); if (upKey.isDown) player.y-=2; if 
(downKey.isDown) player.y+=2; if (leftKey.isDown) player.x-=2; if (rightKey.isDown) player.x+=2; if(ispravanUnos) 
game.physics.arcade.collide([floor1, floor2, platform1], zeleni, collisionHandler, null, this);
}
function render() {
}
